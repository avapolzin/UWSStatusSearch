...
def survey(highlight_fields = False, text_label = True, save_path = False, **kwargs):
	...
	fig= plt.figure(figsize=(((max(coords.ra.deg) - min(coords.ra.deg))/(max(coords.dec.deg) - min(coords.dec.deg)))*10, 15), **kwargs)
	ax = plt.subplot(projection='mollweide')
	ax.grid(color='gray', ls='dashed')
	ax.scatter(coords.ra.radian[UWobs.Nobs.values == 0] - np.deg2rad(180), coords.dec.radian[UWobs.Nobs.values == 0], 
		s = 130., c='lavenderblush', label = r'N$_{obs}$ = 0')
	ax.scatter(coords.ra.radian[np.logical_and((UWobs.Nobs.values > 0),(UWobs.Nobs.values < 100))] - np.deg2rad(180), 
	           coords.dec.radian[np.logical_and((UWobs.Nobs.values > 0),(UWobs.Nobs.values < 100))], s = 130., c='goldenrod', 
	           label = r'0 < N$_{obs}$ < 100')
	ax.scatter(coords.ra.radian[UWobs.Nobs.values >= 100] - np.deg2rad(180), coords.dec.radian[UWobs.Nobs.values >= 100], 
		s = 130., c='seagreen', label = r'N$_{obs}$ >= 100')

	ax.text(np.deg2rad(230), np.deg2rad(-75), 
		s = 'Survey is %i%% completed as of %s.'%((len(UWobs[UWobs.Nobs >= 100])/len(UWobs))*100, last_update), fontsize = 20)

	if highlight_fields:
		for i in highlight_fields:
			highlight_ra = coords.ra.radian[UWobs.ID == i] - np.deg2rad(180)
			highlight_dec = coords.dec.radian[UWobs.ID == i]
			ax.scatter(highlight_ra, highlight_dec, marker = 'x', s = 130, c = 'k')
			if text_label:
				ax.text(highlight_ra - np.deg2rad(0), highlight_dec + np.deg2rad(1), 
					s = '%s'%i, color = 'k', fontsize = 15)

	tick_labels = np.array([30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330])
	ax.set_xticklabels(tick_labels) 
	ax.legend(loc = 'best', fontsize = 25)
	ax.set_xlabel('Right Ascension (deg)', fontsize = 30)
	ax.set_ylabel('Declination (deg)', fontsize = 30)
...
def field(field, highlight_coords = False, save_path = False, virial = False, 
	...
	## NASA Sloan ATLAS
	dist_ = z_conv*NSA_table['ZDIST'].values[np.logical_and((abs(coords[UWobs.ID == field].ra.deg - NSA_table['RA']) <= deg_cut), 
	...
	RA_ = NSA_table['RA'].values[np.logical_and((abs(coords[UWobs.ID == field].ra.deg - NSA_table['RA']) <= deg_cut), 
	   (abs(coords[UWobs.ID == field].dec.deg - NSA_table['DEC']) <= deg_cut)) & (z_conv*NSA_table['ZDIST'] >= dist_limits[0]) & (z_conv*NSA_table['ZDIST'] <= dist_limits[1])]
	Dec_ = NSA_table['DEC'].values[np.logical_and((abs(coords[UWobs.ID == field].ra.deg - NSA_table['RA']) <= deg_cut), 
	   (abs(coords[UWobs.ID == field].dec.deg - NSA_table['DEC']) <= deg_cut)) & (z_conv*NSA_table['ZDIST'] >= dist_limits[0]) & (z_conv*NSA_table['ZDIST'] <= dist_limits[1])]
	
	masses_ = h**2 * NSA_table['MASS'].values[np.logical_and((abs(coords[UWobs.ID == field].ra.deg - NSA_table['RA']) <= deg_cut), 
	   (abs(coords[UWobs.ID == field].dec.deg - NSA_table['DEC']) <= deg_cut)) & (z_conv*NSA_table['ZDIST'] >= dist_limits[0]) & (z_conv*NSA_table['ZDIST'] <= dist_limits[1])]
	...
	if virial:
		...
		ax.set_title(field + ', %.2f percent covered'%(area/6 * 100))
	...
	ax.scatter(RA_[masses_ >= 10**10], Dec_[masses_ >= 10**10], color = 'k', marker = 'X', s = 150)
	...
	if highlight_coords:
		hcs = SkyCoord(highlight_coords, unit = ['hr', 'deg'])
	...
	if save_path:
		fig.savefig(save_path + field + '.png', bbox_inches = 'tight')
	...
	if verbose:
		...
		print('\n\n')
		print('The known galaxies in this field at distances between %.1f and %.1f Mpc are:\n'%(dist_limits[0], dist_limits[1]))
		print(df.to_string())
		print('\n\n')
